...
searchButton?.addEventListener('click', async () => {
    ...
    ul.id = "lista"
    for (const item of (<any>listaDeFilmes).results) {
...
function validateLoginButton() {
    if (password && username && apiKey && loginButton) {
        (<HTMLButtonElement>loginButton).disabled = false;
    } else {
        (<HTMLButtonElement>loginButton).disabled = true;
    }
}
...
class HttpClient {
    static async get({url, method, body = null }: RequestParameters) {
...
async function criarRequestToken() {
    ...
    requestToken = (<RequestParameters["body"]>result).request_token
}
...
async function criarSessao() {
    ...
    sessionId = (<RequestParameters["body"]>result).session_id;
}
